# P5 Work Management Automation Tasks

## Planning

- [x] Read repository authority and programme P5 specification.
- [x] Classify the slice as Complex.
- [x] Create governed change `057-work-management-automation` and isolated worktree.
- [x] Run the clean baseline repository verifier.
- [x] Record approved P5 design decisions, requirements, acceptance, risks, and recovery.
- [x] Create the requirement-mapped implementation plan.

## Implementation

- [x] Task 1 — strict settings, schema, and modularity baseline.
- [x] Task 2 — atomic review evidence persistence.
- [x] Task 3 — reconciliation, service facade, and portfolio status.
- [x] Task 4 — narrow GitHub Project adapter.
- [x] Task 5 — fixed-shape CLI, CI gates, workflow descriptors, and platform composition.
- [ ] Task 6 — documentation, review, commissioning, PR, merge, reconciliation, and cleanup.

## Required final evidence

- [ ] Focused test suites pass on the final implementation.
- [ ] Change scope and governance checks pass.
- [ ] Findings-first review has no unresolved blocking findings.
- [ ] Canonical `scripts/verify.ps1` passes on the exact reviewed head.
- [ ] GitHub live read-only commissioning evidence is recorded.
- [ ] PR exact head is merged and post-merge documentation reconciliation completes.
- [ ] Governed cleanup completes without force or permanent deletion.
