...
- **REQ-001**: canonical long operations must negotiate Tasks transparently: task-backed when the request advertises Tasks, synchronous on the same public operation when it does not.
...
- **REQ-007**: preserve optional explicit sync aliases for compatibility, but do not require callers to select them; live ChatGPT evidence has proved that some requests omit Tasks capability, so the canonical operation itself must fall back synchronously.
