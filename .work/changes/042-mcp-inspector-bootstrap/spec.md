...
- **REQ-005 — Local launcher**: Accept exactly `operation` or `development`; resolve the target port from `settings/kis-mcp.settings.json`; require the selected instance to be configured; bind Inspector to `127.0.0.1`; use distinct configured Inspector web ports; and launch an ad-hoc streamable-HTTP target for `http://127.0.0.1:<instance-port>/mcp`.
