...
4. Exhaust the provider-native PR source-commit SHA list, exclude every such SHA from merge candidacy, then resolve the merge SHA from the registered default-branch commit stream only when the exact PR number/head branch pair and provider-native GitHub `web-flow` committer identity agree, with Git committer time used only as a sanity bound: sub-minute drift from PR `merged_at` is accepted and a one-minute-or-greater difference is rejected as inconsistent provider evidence. Then require provider `changed_files` to be positive and within the 3,000-file GitHub commit ceiling, and page exact merge files only until that count is reached. Reject source-commit impersonation, provider identity disagreement, duplicate/non-progressing pages, early short pages, and count/shape disagreement as retryable provider evidence before selecting the scope path.
