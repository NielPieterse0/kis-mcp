...
- Production scope is the registered-Project semantic reader/commissioner plus focused regressions and this change record.
- Do not absorb #278, #241/#251, #258, stale cleanup, or unrelated repository work.
...
- **REQ-011**: Saved-view pagination MUST be followed explicitly through GitHub `after` cursors under a fixed evidence budget. Cursor cycles, malformed links, malformed pages, provider failures, or budget exhaustion MUST remain unverified and expose bounded reason codes rather than silently truncating to ready.
...
3. Behavioral verification requests at most 100 items per page, follows GitHub `after` cursors explicitly within a fixed 10-page / 1000-item evidence budget, rejects malformed/cyclic/incomplete pagination as unverified, never uses unbounded pagination, rejects malformed/blank evidence, and evaluates only the supported canonical filter grammar.
...
7. #270 remains open until exact-head CI, merge, fresh-runtime commissioning, evidence-backed legacy-state reconciliation, and final acceptance are complete.
...
- Provider or response-shape failures must remain distinguishable from semantic contradictions so commissioning does not issue unnecessary view mutations.
...
- Any implementation or closeout work belonging to other active lanes.
