...
2. Create the authoritative local change first with `pwsh -File scripts/change-workflow.ps1 new <change-id> --outcome <text> --owned <path> --complexity <small|medium|large>`, adding zero or more `--risk-trigger <trigger>` values plus scope/dependency arguments as required. GitHub Issue/Project linkage is optional projection metadata and may be supplied at intake or reconciled later.
...
5. Scale base lifecycle artifacts only by complexity: `small` uses `scope.json` plus `change.md`; `medium` and `large` use `scope.json`, `spec.md`, `plan.md`, `tasks.md`, and `closeout.md`. Risk triggers add only their applicable safeguards/reviews and never increase complexity by themselves.

Complexity is objective workload/coordination evidence, not risk: `small` is one bounded straightforward outcome with no material design or interface redesign; `medium` has several dependent steps or bounded design/shared-interface work in a contained area; `large` spans components/system boundaries, independently reviewable subwork, substantial state/architecture change, a new integration, or significant release/commissioning coordination. File count is supporting evidence only and never the classifier. Reclassify only when discovery materially expands scope or coordination.

Newly created scopes use schema version 4 with `complexity` plus additive `risk_triggers`. Allowed triggers are `security`, `secrets`, `sensitive_data`, `money`, `persistent_state`, `migration`, `external_action`, `deployment`, `destructive`, `public_contract`, and `architecture_boundary`; they are workflow/evidence requirements only and never become a fourth Work hard rule. Historical schema-version-1/2/3 scopes remain valid and retain their original compatibility rules; schema-version-3 records retain their recorded `risk_profile` only as historical compatibility data. Local change governance performs no GitHub or other network call.
