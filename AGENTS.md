...
6. Work Management may own operational priority, Ready/hold/defer state, scheduling, and execution claims. It does not create or override local change, Git, verification, or merge authority. Coordinator-issued implementation packets bind one stable task/packet lineage to a generation-specific run, executor/profile, exact project/change/worktree/base/scope/lifecycle envelope, authority references, and current reservation/lease fence; reassignment must increment generation, create a new run, retain predecessor lineage, and invalidate the prior run for mutation.
