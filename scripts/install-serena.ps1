...
if ($Mode -eq 'PrepareInstall') {
    ...
    $CandidateExecutable = Join-Path $CandidateVenv 'Scripts\python.exe'
    if (-not (Test-Path -LiteralPath $CandidateExecutable -PathType Leaf)) {
        throw "SERENA_CANDIDATE_INTERPRETER_MISSING: $CandidateExecutable"
    }
    & $CandidateExecutable -c 'import serena; from serena.cli import top_level'
    if ($LASTEXITCODE -ne 0) {
        throw "SERENA_CANDIDATE_CLI_IMPORT_FAILED: Python exited with $LASTEXITCODE."
    }
