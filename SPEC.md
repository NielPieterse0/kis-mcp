...
- a lifecycle-owned housekeeping runtime that activates only on `kis-op`, schedules unattended preview-only `work-management-reconciliation` and `backlog-readiness` runs through the parent authenticated provider session, persists bounded atomic receipts/failures/freshness beneath the KIS state root, and permits apply only from a fresh unchanged preview with a deterministic plan-derived idempotency key; the legacy Work Management `scheduled_reconciliation` automation flag remains disabled because this explicit runtime is the scheduler authority;
