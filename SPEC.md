...
- six approval-gated exact registered-GitHub operations for immutable commit publication, tree-equivalent remote-default-rooted review-branch reconciliation, exact-head pull-request creation, merge-commit-only repository landing-policy configuration, exact-head pull-request merge, and exact-head non-default remote-branch deletion, exposed as discoverable virtual operations through `execute_external_action` with exact remote-state verification;
- `execute_change_workflow` for risk-scaled verification/review execution (`lean`, `standard`, `rigorous`) and `prepare_reviewable_pull_request` for fixed verified-source-commit → exact-tree reconciliation → deterministic-metadata open-PR coordination; completion preserves both source and reconciled head SHAs and intentionally stops before merge, remote-branch deletion, or worktree cleanup;
...
`settings/work-management/github-project-schema.json` remains the repository-owned desired operational projection: **18 core fields and 12 named views** from the approved programme. `project_management_schema_status` compares the live Project field inventory with that manifest deterministically. The approved GitHub MCP surface can read Project, field, item, and status-update detail and perform bounded batch item-field updates, but it cannot create generic custom fields, change single-select option schemas, create saved views, or configure native Project workflows. Project/item deletion, Project creation, status-update creation, and iteration-field creation remain outside the approved KIS Project mutation surface. View observability is therefore reported explicitly rather than inferred.
