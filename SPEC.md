...
- a separate lifecycle-owned post-merge commissioning observer that schedules only on `kis-op`, discovers bounded merged-PR candidates through the authenticated GitHub provider, re-verifies provider `merged=true`, resolves the exact default-branch merge commit and exact landed schema-v4 change scope, classifies configured live surfaces from changed paths plus governed risk triggers, and idempotently creates at most one commissioning issue per normalized repository / exact merge SHA / surface; first activation establishes a current-time checkpoint with no historical backfill, failed or ambiguous evidence never advances past unaccounted work, and `kis-dev` exposes only the same read-only observer status/receipt diagnostics without scheduling mutations;
...
`settings/post-merge-commissioning.settings.json` independently owns the post-merge observer policy. Its strict target/surface contract defines the `kis-op` host, polling/overlap/read/mutation bounds, ambiguous governed risk triggers, live-surface path/risk matchers, runtime refresh rule, verification procedure, invariant, evidence target, and terminal success criterion. Candidate search is discovery only: each PR is re-read and must report `merged=true`; the exact merge SHA comes from one matching default-branch merge commit, not the PR head. The observer then reads the exact landed `.work/changes/<change-id>/scope.json` at that merge SHA and requires schema-v4 Work identity consistency before classification. Stable keys are `commission:<owner/repo>:<merge-sha>:<surface-id>`, duplicate detection searches all issue states, and source delivery is never reopened or rewritten. `kis_post_merge_commissioning_status` and `kis_post_merge_commissioning_receipt` are read-only diagnostics on both instances; only `kis-op` schedules the observer. Initial activation establishes the current-time checkpoint and performs no historical backfill; #455 owns explicit backfill, while #454 owns execution of generated commissioning work and projection of `Live Verification` evidence.
