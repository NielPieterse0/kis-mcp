...
- Context7, Control Center, GitHub MCP, Serena, and Supabase adapters mounted under `context7_*`, `controlcenter_*`, `github_*`, `serena_*`, and `supabase_*` when enabled adapters build successfully;
...
The Provider registry contains Control Center, Desktop Commander, Context7 MCP, GitHub MCP, NVIDIA NIM, Serena MCP, and Supabase descriptors. Runtime JSON selects Context7, Control Center, GitHub, Serena, and Supabase for deterministic namespaced mounting. Adapter build, invalid-result, mount, authentication, and commissioning states remain separate; one unavailable optional provider does not prevent Work, Discover, Skills, agent registration, or gateway startup.
