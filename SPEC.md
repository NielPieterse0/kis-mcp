...
- four approval-gated exact registered-GitHub operations for immutable commit publication, tree-equivalent remote-default-rooted review-branch reconciliation, exact-head pull-request merge, and exact-head non-default remote-branch deletion, exposed as discoverable virtual operations through `execute_external_action` with exact remote-state verification;
