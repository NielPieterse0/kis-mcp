...
- `settings/work-management/github-projects.settings.json` owns Work Management feature, gate, automation, evidence, and backend-binding behavior; `settings/work-management/github-project-schema.json` owns the desired 18-field/12-view GitHub Project operational projection used for deterministic drift evidence.
...
- provider-neutral Work Management with typed records, documentation-aware actionable intake, exact implementation traceability, atomic review evidence persistence, deterministic reconciliation, attributable portfolio status, repository-owned Project schema/drift evidence, fixed-shape CLI/CI, and seven task-level platform workflows;
- a bounded GitHub Project adapter that reads configured Projects and field schema, adds existing issues or pull requests, updates explicit item fields, preflights revisions, deduplicates source records, and exposes no delete or unrestricted GraphQL operation;
- pre-merge `project_management_merge_readiness` gate evaluation and post-merge `project_management_documentation_reconcile` orchestration using the existing `documentation_reconciliation_due` / `post_merge_complete` lifecycle, with the Work Management closeout workflow required to honor an unready result and required work remaining in `Documentation` until reconciliation evidence is complete;
...
The Provider registry contains nine descriptors: Control Center, Desktop Commander, Context7 MCP, DBHub, Docker Hub MCP, GitHub MCP, NVIDIA NIM, Serena MCP, and Supabase. Runtime JSON selects seven providers for deterministic namespaced mounting: Context7, Control Center, DBHub, Docker Hub MCP, GitHub, Serena, and Supabase. DBHub uses the source-aware connector boundary and one isolated child process per registered binding; the checked-in College `results` SQLite binding is configured and locally present, but the pinned DBHub installation is not yet commissioned. Docker Hub is configured in credential-free public mode with no invented project namespace binding and remains separate from local Docker Engine operations; its pinned installation is likewise not yet commissioned. Adapter build, invalid-result, mount, authentication, and commissioning states remain separate; one unavailable optional provider does not prevent Work, Discover, Skills, agent registration, or gateway startup.
...
The work-management domain remains provider-neutral and does not import FastMCP or GitHub layouts. Platform composition adds eight bounded task-level tools when strict work-management settings are enabled: inventory, reconciliation, Project schema status, merge readiness, documentation reconciliation, portfolio status, review-evidence persistence, and traceability verification. Seven task-level workflow descriptors compose those operations. The checked-in configuration is enabled for the `kis-mcp` user Project #1 binding. Feature, gate, automation, and evidence modes remain owned by work-management settings; managed-project local/GitHub identity and Project coordinates are reconciled from the central registry, while existing backend-binding IDs remain stable for compatibility.

`settings/work-management/github-project-schema.json` is the repository-owned desired operational projection: 18 core fields and 12 named views from the approved programme. `project_management_schema_status` compares the live Project field inventory with that manifest deterministically. The current approved GitHub MCP surface can list fields and update item values but cannot create generic custom fields, change single-select option schemas, create saved views, or configure native Project workflows. View observability is therefore reported explicitly rather than inferred. The one exposed schema-creation primitive, iteration-field creation, is not used because the approved programme leaves iteration cadence optional and no recoverable delete-field path is exposed.

Local review evidence is persisted only beneath `.work/reviews/<review-id>/` using bounded atomic replacement and conflict detection. Remote reconciliation defaults to preview and requires explicit apply plus an idempotency key. Safe create checks observed Project items for the same source issue or pull request before mutation, so restart does not duplicate records. Actionable task/specification intake requires documentation-impact classification. Pre-merge readiness consumes exact PR/head verification evidence plus the Work record and returns an explicit gate result; the Work Management closeout workflow must not proceed to merge when that result is unready. After confirmed merge evidence, an operator or governing workflow invokes `project_management_documentation_reconcile` to create/apply `documentation_reconciliation_due`; required work remains in `Documentation` until a later invocation records `post_merge_complete` at an exact completion revision. Provider absence, unsupported methods, stale revisions, inaccessible state, incomplete pagination, and registry/work-management identity conflicts are corrective work-management outcomes rather than HR policy violations.

Current live commissioning status belongs to `docs/OPERATIONS.md`; detailed dated evidence is retained in `docs/development/github-project-onboarding/commissioning.md` and change 110. Repository implementation of schema/drift and documentation-lifecycle integration is complete, while live rich Project schema/view provisioning remains incomplete because the approved bounded connector lacks those mutation primitives. Repository and Project authorization are validated through registered GitHub coordinates; historical repository-local routing settings remain compatibility data and do not widen those registered coordinates.
