...
| Execution substrate | Defines provider-neutral execution request/result/readiness/lifecycle contracts. Verification continues to use `local-process` by default; a disabled-by-default `windows-hyperv-proof` profile provides an internal exact-source disposable Windows proof path with bounded provenance/evidence and fail-closed lifecycle outcomes. Live Hyper-V commissioning is separate evidence and is not implied by profile availability. |
...
- `settings/execution-runners.settings.json` defines execution backend/profile selection and provenance requirements. `local-process` remains the default verification backend; `windows-hyperv-proof` is disabled by default and requires explicit readiness plus exact source/image/profile/toolchain identity before disposable guest execution.
...
- execution contract/schema identity, backend selection, exact-source/provenance binding, evidence bounds, fail-closed lifecycle outcomes, local-process compatibility, and disposable Hyper-V proof behavior without treating unavailable live Hyper-V commissioning as a passing result.
...
- provider-neutral execution request/result/readiness/lifecycle contracts, `local-process` as the default verification backend, and a disabled-by-default internal `windows-hyperv-proof` profile that binds exact source/image/profile/toolchain identity and returns bounded fail-closed evidence; this profile is implemented but is not live-commissioned on the current non-Hyper-V host;
