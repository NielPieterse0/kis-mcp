...
When Work is successfully claimed into `Active`, the Work boundary automatically materializes one immutable task handoff from the source issue, atomically assigns its permanent non-reused localhost candidate port, and establishes the durable evidence/candidate namespace. Re-entry for the same Work identity reuses the exact contract and port or fails closed on changed contract content. Candidate runtime receipts bind PID, server-instance identity, contract fingerprint, source identity, and the actual governed source path; candidate reuse requires the live endpoint to prove that exact identity, and shutdown is permitted only for that exact owner after its live-candidate proof is durably present. Implementation/review evidence, live candidate proof, provider exact-head proof, merge evidence, and post-merge promotion receipts extend the same evidence lineage by reference with class-specific validity inputs.

Selected long-running operations use `io.modelcontextprotocol/tasks` execution. `review_change_with_agent`, `kis_post_merge_commissioning_run`, and `converge_change_to_done` remain task-optional. `prepare_reviewable_pull_request` is task-required so its verification, registered publication, and PR-creation sequence cannot silently run as one long synchronous HTTP request; `prepare_reviewable_pull_request_sync` is the explicit compatibility surface for clients that do not implement Tasks. A Tasks client receives a `resultType: "task"` handle only after durable task creation and may retrieve the same terminal result by task ID after reconnecting to the same running KIS service. MCP task IDs are transport-facing handles only; existing KIS Work records, stable completion operation identity, registered GitHub idempotency/reconciliation, receipts, and fencing remain authoritative and prevent transport replay from creating duplicate mutations. `converge_change_to_done` consumes a persisted `PromotionReady` handoff and uses the durable KIS `PromotionController` checkpoint as the sole resume authority for the ordered promotion stages; completed stages and their observations survive request/task interruption and are not replayed merely because MCP task state is lost.

A blocked promotion stage also persists any authoritative external identity already discovered before the block. Exact-head Actions therefore binds one qualifying PR workflow-run ID and polls that run directly on resume instead of rescanning branch history. Initial Actions discovery is bounded to one workflow-scoped page for the exact review branch/head; the Actions detail read validates the exact head and PR association, so promotion does not separately reread the PR solely to reconstruct that fact. PR creation reuses the title/context already carried by the authoritative Work record rather than rereading the source issue. Registered commit reconciliation owns review-branch observation, avoiding a duplicate outer branch-head read. Post-merge default inclusion uses exact default/merge equality when possible and otherwise a conclusive local `git merge-base --is-ancestor` assertion against the exact default revision fetched by registered refresh, avoiding paginated provider history traversal. This local assertion runs only against the governed `.work/worktrees` checkout already validated by the once-through source-binding boundary, and therefore relies on the repository's existing Git CLI runtime dependency rather than introducing a new trust source. When required post-merge updates are empty, documentation reconciliation creates and completes the no-op milestone in one logical call while preserving the normal documentation lifecycle internally. An uncertain exact-head merge is reconciled from current pull-request truth before another merge mutation is attempted, and a completed Work transition is retained while a failed source-issue closure resumes independently. After cleanup succeeds, the controller persists one `promotion-terminal-receipt-v1` record binding the Work/change/source revision, PR/head, Actions run, merge commit, landed revision, documentation completion, Work/source closeout, cleanup observations, generated closeout projection, explicit implementation-source-to-provider-head verification lineage, and durable workflow telemetry. Telemetry records stage attempts/timings, replay and block reasons, provider reads/mutations, bounded list-page scans, operation counts, verification/review activity, and duplicate-attempt indicators; replay of the same completed operation performs no lifecycle mutation and increments durable replay telemetry. The bounded read-only `workflow_terminal_audit` surface inspects recent terminal receipts directly, exposes exact terminal identities/delivery evidence and historical tracked-change role, and flags missing identity, repeated provider discovery, duplicate review/verification/proof attempts, restart mismatch, and provider-call budget regressions without reconstructing provider state. Targeted Work-board queries propagate through Work Management to the GitHub Project provider before `item_limit`, preventing an unrelated earlier page from hiding a known exact record. Promotion validates handoff/source/typed Work identity before provider activity and still consumes established implementation review evidence rather than initiating another substantive KIS review. `prepare_reviewable_pull_request` consumes matching PromotionReady execution evidence when supplied and does not rerun implementation verification or substantive review; canonical full repository verification remains owned by provider-native Actions on the exact PR head.
...
`settings.remote_mcp` defines exactly two local instances: internal `operation`, exposed in ChatGPT as `kis-op` on `127.0.0.1:8010`, and internal `development`, exposed as `kis-dev` on `127.0.0.1:8011`. Each record has a distinct app name, port, profile name, tunnel ID, vault secret reference, runtime directory, and explicit `configured` state. Selection is explicit through the launcher instance argument or the JSON `active_instance`; both instances may run concurrently. Each ready runtime generation installs an instance-scoped local health guard. If that same generation remains the authoritative `current.json` owner after a bounded health-failure grace period, the guard invokes the independent local-shell `recover-chatgpt.ps1` surface for that instance only. A transient recovery failure is retried with capped exponential backoff instead of silently ending supervision; once recovery succeeds, the replacement launcher installs the next generation's guard. Superseded generations exit without recovery and the peer instance is never selected implicitly.
