...
The GitHub connector is commissioned through the official binary and supervised OAuth flow with persistent credential reuse and authenticated private-repository read evidence. The Supabase connector implements hosted OAuth 2.1 dynamic client registration, Windows Credential Manager persistence, mandatory project scoping, harmless `get_project_url` verification, and shared `supabase_*` smoke support without PAT transport. Supabase authentication and live shared-runtime evidence remain unverified until the operator completes the browser authorization and commissioning commands for a development or test project.
