...
- provider-neutral execution request/result/readiness/lifecycle contracts, `local-process` as the default verification backend, disabled-by-default internal `windows-virtualbox-proof` and `windows-hyperv-proof` profiles that bind exact source/image/profile/toolchain identity and return bounded fail-closed evidence, and one backend-neutral disposable-verification proof adapter; the VirtualBox provider forces KIS-owned global/config/template-media/clone state, pre-start host-integration and network isolation, password-file credentials, exact archive injection through Guest Additions, and recoverable quarantine retirement. Neither disposable Windows provider is treated as live-commissioned by implementation alone;
