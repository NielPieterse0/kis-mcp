...
- a separate lifecycle-owned post-merge commissioning runtime whose observer schedules only on `kis-op`, discovers bounded merged-PR candidates through the authenticated GitHub provider, re-verifies provider `merged=true`, resolves the exact default-branch merge commit and exact landed schema-v4 change scope, classifies configured live surfaces from changed paths plus governed risk triggers, idempotently creates at most one commissioning issue per normalized repository / exact merge SHA / surface, and projects required/not-required/ambiguous classification into only the canonical source live-verification fields; first activation establishes a current-time checkpoint with no historical backfill and incomplete provider/projection work does not advance the checkpoint. The same runtime exposes read-only status/receipt/execution diagnostics plus an approval-required explicit runner that revalidates exact issue/merge/classifier/Work-claim identity, gates stale runtime generations without self-restart, executes one closed read-only probe profile, persists resumable per-attempt and aggregate evidence, and completes/blocks/leaves-open commissioning Work according to Passed/Blocked/Failed while source delivery remains independent;
