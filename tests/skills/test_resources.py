...
def test_catalogue_index_is_bounded_and_has_deterministic_continuation(
    skills_config, make_skill
) -> None:
    for skill_id in ("alpha-skill", "beta-skill", "gamma-skill"):
        make_skill(skill_id)
    limited = replace(
        skills_config,
        limits=replace(
            skills_config.limits,
            list_default_limit=1,
            list_max_limit=2,
        ),
    )
    server = _server(SkillCatalogue(limited))

    first = json.loads(_read(server, "skill:///"))
    second = json.loads(_read(server, first["next_uri"]))

    assert [item["skill_id"] for item in first["skills"]] == ["alpha-skill", "beta-skill"]
    assert first["skill_count"] == 3
    assert first["truncated"] is True
    assert first["next_uri"].startswith("skill:///catalogue?cursor=")
    assert [item["skill_id"] for item in second["skills"]] == ["gamma-skill"]
