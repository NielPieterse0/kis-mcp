...
def test_main_dispatches_backlog_readiness_runner(monkeypatch) -> None:
    args = argparse.Namespace(
    ...
    parser = SimpleNamespace(parse_args=lambda: args)
    monkeypatch.setattr(housekeeping, "_parser", lambda: parser)
    monkeypatch.setattr(housekeeping, "build_server", lambda: object())
    monkeypatch.setattr(housekeeping, "FastMCPInvoker", lambda server: server)

    async def fake_runner(invoker, config, trigger):
        assert config.repository == "NielPieterse0/kis-mcp"
        assert trigger.runner.value == "backlog_readiness"
        return SimpleNamespace(
            complete=True,
            to_json_dict=lambda: {"complete": True, "runner": trigger.runner.value},
        )

    monkeypatch.setattr(housekeeping, "run_backlog_readiness", fake_runner)
