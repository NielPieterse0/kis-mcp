...
def test_accepts_exact_handoff_consumes_key_and_replays_idempotently(tmp_path: Path) -> None:
    ...
    assert first["verification_requirements"]["verification_authority"] == "kis_local_exact_head"
...
def test_verification_requirements_use_local_exact_head_contract(tmp_path: Path) -> None:
    ...
    assert requirement["schema_version"] == 2
    assert requirement["contract"] == "coordinator-verification-requirements-v2"
    assert requirement["verification_authority"] == "kis_local_exact_head"
...
def test_integration_queue_requires_exact_local_verification(tmp_path: Path) -> None:
    ...
    with pytest.raises(ReservationAdmissionError, match="LOCAL_EXACT_HEAD_VERIFICATION_REQUIRED"):
        queue.authorize_delivery(
            item["queue_item_id"],
            verification={"revision": "9" * 40, "status": "passed", "source": "local", "reference": "x"},
        )
...
def test_integration_queue_rechecks_authority_before_admission_and_delivery(
    ...
    queue.authorize_delivery(
        ...
        verification={
            ...
            "status": "passed",
            "source": "local",
            "reference": "local:verify@authority-check",
        },
    )
