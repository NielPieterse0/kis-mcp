...
class ProviderStdioCommand:
    ...
    def __post_init__(self) -> None:
        ...
        if any(_ENVIRONMENT_NAME.fullmatch(name) is None for name in environment_names):
            raise ValueError("environment variable names must use upper-case shell syntax")
        object.__setattr__(self, "executable", executable)
        object.__setattr__(self, "arguments", arguments)
        object.__setattr__(self, "environment_names", tuple(sorted(environment_names)))


__all__ = ["ProviderStdioCommand"]
