...
For each instance, the script starts the settings-defined loopback streamable HTTP endpoint, establishes the supported MCP discovery boundary, lists tools, calls `kis_health`, calls bounded `inspect_project` against the repository root, verifies representative read/write/edit/process tools, writes and reads a unique marker beneath `C:\Projects\.kis-mcp\temp`, and quarantines the marker recoverably. Modern MCP 2026 requests use request-scoped metadata and `server/discover`; legacy initialize behavior is compatibility-only. `give_feedback_to_desktop_commander` remains absent because every invocation is external-network-only; ordinary mixed-purpose tools, `inspect_project`, and `inspect_change` remain exposed.
...
There is no cross-instance failover or cross-instance process ownership. Each ready generation starts an instance-scoped health guard. If the same `current.json` generation remains expected-running after a bounded health-failure grace period, the guard invokes local-shell recovery for that instance only. Transient recovery failure is retried with capped exponential backoff so the guard does not silently disappear after one failed restart; a successful replacement launcher installs the next generation's guard. A superseded generation exits without recovery.

Use the independent local-shell recovery surface when either MCP/tunnel path cannot recover itself:
...
Both wrappers delegate to `recover-chatgpt.ps1` with a fixed instance. Recovery is idempotent: a healthy matching runtime is reused; otherwise a selected-instance launcher is detached and local MCP plus tunnel readiness are verified before success. Neither wrapper selects the configured active instance or the peer. Use `-Foreground` only when an attached recovery console is preferable for diagnosis.
