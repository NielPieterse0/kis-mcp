...
- `settings/work-management/github-projects.settings.json` for Work Management feature/gate/automation/evidence modes and stable backend bindings, and `settings/work-management/github-project-schema.json` for the desired 20-field/12-view Project projection used by schema-drift checks, including `Complexity` and `Risk Triggers` as change-classification projections.
...
For registered repositories, `kis_github_publish_registered_commit` publishes one existing immutable local commit to a named branch only when the observed remote ref exactly matches `expected_remote_base` (or is absent when that value is null), then verifies the remote ref equals the exact local commit SHA. When local merge history is not an ancestor of current GitHub history but the declared local source-base tree is exactly identical to the expected remote-default tree, `kis_github_reconcile_registered_commit` can instead create a non-default review-branch commit that preserves the exact source tree and uses the verified remote-default SHA as its sole parent. `kis_github_create_registered_pull_request` verifies exact head/base state before and after creation. These three pre-review mutations accept a bounded `deadline_ms` and a read-only `status_only` mode. They return a deterministic `operation_id`, `elapsed_ms`, and one of `not_started | in_progress | applied | failed | unknown`; timeout acknowledgement loss is reconciled against GitHub authority before the caller is told whether retry is safe. Repeating the same exact intent retains the same operation ID even when deadline/status controls change, and an already-applied exact branch/PR is recovered without a duplicate mutation. No durable receipt store is introduced here: future persisted operation state must use the state-ownership contract defined by #278.

`kis_github_configure_registered_repository` configures one registered repository for merge commits only, disables squash/rebase landing, keeps GitHub automatic branch deletion disabled, and verifies those settings after mutation. `kis_github_commission_registered_project_schema` resolves one exact central-registry Project binding and the checked-in schema manifest, creates only missing manifest fields/options/views, preserves existing single-select option IDs, refuses type/layout mismatches and deletion, and re-reads the Project before reporting `ready=true`. `kis_github_merge_registered_pull_request` accepts only `merge` and merges only the explicitly approved pull-request head with `--match-head-commit`; `kis_github_delete_registered_branch` refuses the default branch and deletes only an exact expected head with a ref lease. These operations do not expose arbitrary `gh`, shell, GraphQL/REST input, commit messages, force-rewrite, admin-bypass, or token access.

The KIS speculative landing queue is configured by `settings/github-merge-queue.settings.json`. V1 is deliberately narrow: target `main`, merge commits only, FIFO, ALLGREEN, build concurrency 3, merge group 1-3, zero merge wait, no queue jump, squash, rebase, or HEADGREEN. Generated state lives below `C:\Projects\.kis-mcp\merge-queue` and is rebuildable; GitHub/Git remain authoritative. Use `kis_github_merge_queue_status` for read-only queue/base drift inspection. Approval-gated `kis_github_merge_queue_enqueue` freezes an exact PR head; `kis_github_merge_queue_reconcile` validates live PR/base identity, builds cumulative two-parent merge commits, publishes generation-scoped `kis-readonly-queue/main/g<generation>/pr-<number>` refs, and binds Actions evidence to the exact candidate SHA; `kis_github_merge_queue_dequeue` removes only the expected frozen head and invalidates the generation; `kis_github_merge_queue_land` advances only the contiguous ALLGREEN prefix after revalidating generation, live base, source heads, membership, candidate checks, and ancestry. Base advancement reuses `kis_github_publish_registered_commit` semantics with the exact generation base as `expected_remote_base`, so a stale lease fails rather than repairing the candidate in place.

Any base movement, source-head/base/draft/open/review/protection-state change, conflict, failed candidate check, or 30-minute unanswered candidate check invalidates affected speculative evidence and rebuilds through a new generation. Candidate CI is the existing canonical workflow triggered by queue-ref pushes; it checks out `github.sha`, so evidence is attached to the candidate commit rather than the source PR head. The queue does not bypass Work Management readiness, required review, or documentation lifecycle. The `complete-work-managed-merge-queue` workflow keeps `project_management_merge_readiness` before enqueue and `project_management_documentation_reconcile` after landing. Run `scripts/smoke-github-merge-queue.ps1` for the bounded local commissioning checks; live commissioning additionally requires a commissioned KIS runtime and registered GitHub repository.

`prepare_reviewable_pull_request` is the bounded completion coordinator for the pre-review phase. Supply a registered `project_id`, exact local `commit` and `source_base` SHAs, a non-default review branch with its exact expected remote SHA/absence, the exact expected remote-default SHA, PR outcome/summary, explicit approval, `small|medium|large` complexity, zero or more approved additive risk triggers, documentation impact, residual state, and optional verification/review overrides. One aggregate `deadline_ms` bounds verification, registered publication, PR creation, and reconciliation; nested operations receive only the remaining budget. The result/error includes a stable `prp-...` operation identity, aggregate elapsed time, per-stage timings, and the same five-state operation vocabulary. Use `reconcile_only=true` after a lost/ambiguous response: KIS first observes the registered review-branch publication without mutation, returns `not_started` immediately when no publication exists, or reconstructs the exact verified PR intent and observes PR authority. A published branch without the exact PR is `in_progress`; an exact open PR is `applied`. Normal execution still requires `execute_change_workflow` to pass before any external mutation, then reconciles the exact tree, generates deterministic PR metadata containing complexity/risk/scope/source+published heads/verification/review/documentation/residual state, and creates the PR only at the returned reconciled head. It deliberately stops with an open reviewable PR; provider-native GitHub Actions evidence for that PR head is the final merge-verification gate.
...
Use `project_management_schema_status` before schema-dependent reconciliation. It reads the official GitHub MCP field surface plus bounded registered-Project view inventory and compares them with `settings/work-management/github-project-schema.json`, whose target is **25 fields and 12 named views with declared layouts**. `Module` is a text field; GitHub requires at least one option when creating a single-select, so an empty `Module` single-select is not a valid target contract. `Blocked By` is a commissioned text field used to distinguish observed no-dependency state from unavailable dependency evidence during deterministic queue admission. Empty Project field entries must read as `null`, never as their field metadata name. When the registered Project is not ready, invoke `kis_github_commission_registered_project_schema` through `execute_external_action` with `project_id="kis-mcp"`, `project_binding_id="work-management"`, and explicit `approved=true`, then rerun schema status. The commissioner uses only the central registry plus the checked-in manifest, preserves existing option IDs, refuses incompatible types/layouts, and exposes no deletion or arbitrary API surface. If the `Iteration` field is missing, its one-time bootstrap configuration uses 14-day iterations beginning on the Monday of the current UTC week; an existing iteration field is never rewritten. Normal Project/item mutation remains with the official provider; Project/item deletion, Project creation, status-update creation, native workflow administration, and unrestricted GraphQL remain outside the approved surface.
...
Current routing uses one shared user Project #1 backend with the stable `github-default` binding. `kis-mcp`, `chatgpt-skill`, `commodity`, `college`, and `import-isolate` are managed repositories; the Project coordinate is registered once through `kis-mcp` while each repository keeps its own central-registry identity. Treat `project_management_schema_status` as the current live commissioning authority rather than relying on historical sparse-schema evidence. If it is not ready, use the bounded registered-Project commissioner above and verify again before running schema-dependent queue/claim transitions. Change 113 backfilled recent slice and residual records through preview-first reconciliation. All custom/native automation remains disabled. Runtime-scoped GitHub OAuth remains a separate supervised connection state.
...
The commissioning command reports installation, configuration, authentication, upstream connection, tool discovery, and live verification separately. After a provider's live tool discovery succeeds, KIS writes one credential-free exact-identity evidence document beneath `C:\Projects\.kis-mcp\commissioning\providers\<provider-id>\<identity-fingerprint>.json`. The identity binds provider revision/entry-point bytes, enabled tool surface, and applicable provider configuration; DBHub also binds its registered database bindings. Re-running commissioning for the same exact identity is idempotent. If the provider identity changes, the prior evidence is simply inapplicable and commissioning is required again.

On a later KIS restart, matching evidence allows provider status to report historical live verification without pretending that the new child process has already re-established an upstream connection or rediscovered tools. `upstream_connected`, `tools_discovered`, mount state, and other current-process fields remain current-runtime evidence. DBHub generates one runtime TOML and starts one isolated child per registered binding. College currently exposes only `db_college_results_search_objects` and `db_college_results_execute_sql`; the latter is forced read-only with the JSON-owned row bound. Current live evidence includes a successful `SELECT 1 AS commissioned` against the College SQLite binding.
...
Create a governed Git change only when version-controlled repository content changes. Pure supervised runtime/operator actions that leave repository authority unchanged may be tracked in Work Management without manufacturing a branch, worktree, or PR. For a repository change, record workload complexity independently from any additive risk triggers; Work Management linkage remains optional projection metadata:
...
Schema-version-4 `new` records the local base commit/tree plus `complexity` and ordered `risk_triggers`, then classifies supplied or locally available remote-tracking evidence as `same_sha`, `tree_equivalent`, `content_divergence`, or `unavailable` without contacting the provider. `small` creates `scope.json` plus `change.md`; `medium` and `large` create the full five-file lifecycle record. Risk triggers add only their applicable safeguards and do not increase complexity. Historical schema-version-1/2/3 scopes remain valid under their original compatibility rules. Generated tracked artifacts use explicit LF bytes on every host so Windows newline translation cannot create a `core.safecrlf` staging failure.
...
Cleanup refuses a dirty worktree or an unmerged branch. It performs only normal `git worktree remove`, `git branch -d`, and `git worktree prune` operations; it never forces deletion. For schema-version-3 and schema-version-4 changes, successful merged cleanup establishes historical closed state without a second repository commit solely to rewrite lifecycle metadata.
